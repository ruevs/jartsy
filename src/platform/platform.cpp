...
// Include Apple headers before jartsy.h to avoid identifier clashes.
...
//#include "config.h"
#if defined(WIN32)
// Conversely, include Microsoft headers after jartsy.h to avoid clashes.
#   include <windows.h>
#   include <shellapi.h>
#else
#   include <unistd.h>
...
std::string Narrow(const wchar_t *in)
{
    ...
    out.resize(len - 1);
    jartsyassert(WideCharToMultiByte(CP_UTF8, 0, in, -1, &out[0], len, NULL, NULL),
             "Invalid UTF-16");
    return out;
}
...
std::string Narrow(const std::wstring &in)
{
    ...
    out.resize(WideCharToMultiByte(CP_UTF8, 0, &in[0], (int)in.length(),
                                   NULL, 0, NULL, NULL));
    jartsyassert(WideCharToMultiByte(CP_UTF8, 0, &in[0], (int)in.length(),
...
std::wstring Widen(const char *in)
{
    ...
    out.resize(len - 1);
    jartsyassert(MultiByteToWideChar(CP_UTF8, 0, in, -1, &out[0], len),
             "Invalid UTF-8");
    return out;
}
...
std::wstring Widen(const std::string &in)
{
    ...
    jartsyassert(MultiByteToWideChar(CP_UTF8, 0, &in[0], (int)in.length(),
...
Path Path::CurrentDirectory() {
    ...
    jartsyassert(length > 0 && length == rawW.length() - 1, "Cannot get current directory");
    rawW.resize(length);
    return From(Narrow(rawW));
#else
    char *raw = getcwd(NULL, 0);
    jartsyassert(raw != NULL, "Cannot get current directory");
    Path path = From(raw);
    free(raw);
    return path;
#endif
...
Path Path::Join(const std::string &component) const {
    jartsyassert(component.find(SEPARATOR) == std::string::npos,
...
std::string Path::ToPortable() const {
    jartsyassert(!IsAbsolute(), "absolute paths cannot be made portable");
...
FILE *OpenFile(const Platform::Path &filename, const char *mode) {
    jartsyassert(filename.raw.length() == strlen(filename.raw.c_str()),
...
const void *LoadResource(const std::string &name, size_t *size) {
    ...
    jartsyassert(hres != NULL, "Cannot find resource");
    HGLOBAL res = ::LoadResource(NULL, hres);
    jartsyassert(res != NULL, "Cannot load resource");
...
static Platform::Path FindLocalResourceDir() {
    ...
    resourceDir = selfPath.Parent().Parent().Join("share").Join("jartsy");
    if(stat(resourceDir.raw.c_str(), &st) != -1) {
...
const void *LoadResource(const std::string &name, size_t *size) {
    ...
    if(it == cache.end()) {
        jartsyassert(ReadFile(ResourcePath(name), &cache[name]), "Cannot read resource");
        it = cache.find(name);
    }
